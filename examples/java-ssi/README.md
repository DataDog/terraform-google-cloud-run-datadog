# Example: Deploying an Instrumented Java Cloud Run Service with Datadog SSI
...
Instrumentation here comes from Single-Language Serverless Instrumentation (SSI): `datadog_apm_instrumentation` makes the module run a tracer sidecar that stages the Java tracer on a shared volume and sets `JAVA_TOOL_OPTIONS` for the app container. Because of that, the image ships no `dd-java-agent.jar` -- two `-javaagent` copies would conflict.

For the variant that installs the tracer in the image instead, see [`examples/java`](../java).
...
| <a name="input_name"></a> [name](#input\_name) | The name of the Cloud Run service | `string` | `"cloud-run-tf-example-java-ssi"` | no |
