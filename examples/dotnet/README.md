# Example: Deploying an Instrumented Dotnet App to Cloud Run with Datadog

This example demonstrates a step-by-step on how to use the `terraform-google-cloud-run-datadog` wrapper module to fully instrument a sample Dotnet application with logs, metrics, and tracing using Datadog. If you're looking for a quick deploy, go to ../README.md for the build and deploy bash script.
