...
Instrumentation here comes from Single-Language Serverless Instrumentation (SSI): `datadog_apm_instrumentation` makes the module run a tracer sidecar that stages the Ruby tracer on a shared volume and sets `RUBYOPT` for the app container. Because of that, the `datadog` gem is not in the `Gemfile`.
