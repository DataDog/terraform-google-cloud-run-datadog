# Example: Deploying an Instrumented Node.js Cloud Run Service with Datadog SSI
...
Instrumentation here comes from Single-Language Serverless Instrumentation (SSI): `datadog_apm_instrumentation` makes the module run a tracer sidecar that stages the Node.js tracer on a shared volume and sets `NODE_OPTIONS` for the app container. Because of that, `dd-trace` is not a dependency of the app and the image sets no `NODE_OPTIONS`.

For the variant that installs the tracer in the image instead, see [`examples/node`](../node).
...
| <a name="input_name"></a> [name](#input\_name) | The name of the Cloud Run service | `string` | `"cloud-run-tf-example-node-ssi"` | no |
