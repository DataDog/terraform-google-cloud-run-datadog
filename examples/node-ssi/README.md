...
Instrumentation here comes from Single Step Instrumentation (SSI): `datadog_apm_instrumentation` makes the module run a tracer sidecar that stages the Node.js tracer on a shared volume and sets `NODE_OPTIONS` for the app container. Because of that, `dd-trace` is not a dependency of the app and the image sets no `NODE_OPTIONS`.
