...
module "datadog-cloud-run-v2-wrapper-node" {
  source = "../../"
  name = var.name
  location = var.region
...
resource "google_cloud_run_service_iam_member" "invoker-node" {
  service  = module.datadog-cloud-run-v2-wrapper-node.name
  location = module.datadog-cloud-run-v2-wrapper-node.location
